...
@objc protocol SingularitySwiftRDelegate{
    /**!
     登录成功的回调
     
     :param: Dict 服务器返回的数据
     */
    optional func SingularitySwiftR_LoginCallBack(request:AnyObject?)
    /**
     获取目标信息回调
     
     :param: request chat_session_id 聊天会话 id target_id 目标id target_name 目标名称 target_picture 目标头像
     */
    optional func SingularitySwiftR_ReceiveTargetInfo(request:AnyObject?)
    /**
     获取用户会话列表的回调
     ...
     */
    optional func SingularitySwiftR_ReceiveChatSessionList(request:AnyObject?)
     ...
     */
    optional func SingularitySwiftR_ReceiveUnreadMessages(request:AnyObject?)
     ...
     */
    optional func SingularitySwiftR_ReceiveHistoryMessages(request:AnyObject?)
     ...
     */
    optional func SingularitySwiftR_MessageCallback(request:AnyObject?)
     ...
     */
    optional func SingularitySwiftR_ReceiveMessage(request:AnyObject?)
     ...
     */
    optional func SingularitySwiftR_ChatUserStatusChanged(request:AnyObject?)
...
public enum SingularitySwiftRState{
    case Connecting
    case Connected
    case Disconnected
}
...
class SingularitySwiftR: NSObject {
    var SingularitySwiftRDaostate:SingularitySwiftRState!
    weak var delegate : SingularitySwiftRDelegate?
    var chatHub:Hub!
    var hubConnection:SignalR!
    /**
     初始化连接池，并监听回调
     */
    func _initSingularitySwiftR(urlStr urlStr:String){
        ...
        hubConnection = SwiftR.connect(urlStr) { [weak self] connection in
            ...
            self?.chatHub.on("loginCallback", callback: { (args) -> () in
                ...
                self?.delegate?.SingularitySwiftR_LoginCallBack!(user)
            ...
            self?.chatHub.on("receiveTargetInfo", callback: { (args) -> () in
                ...
                self?.delegate?.SingularitySwiftR_ReceiveTargetInfo!(mmtargetmodel)
            })
            ...
            self?.chatHub.on("receiveChatSessionList", callback: { (args) -> () in
                ...
                self?.delegate?.SingularitySwiftR_ReceiveChatSessionList!(sessionlistmodel)
            ...
            self?.chatHub.on("receiveUnreadMessages", callback: { (args) -> () in
                ...
                self?.delegate?.SingularitySwiftR_ReceiveUnreadMessages!(unreadMessages)
            })
            ...
            self?.chatHub.on("receiveHistoryMessages", callback: { (args) -> () in
                ...
                self?.delegate?.SingularitySwiftR_ReceiveUnreadMessages!(historyMessageses)
            })
            ...
            self?.chatHub.on("messageCallback", callback: { (args) -> () in
                ...
                self?.delegate?.SingularitySwiftR_MessageCallback!(messagemodel)
            ...
            self?.chatHub.on("receiveMessage", callback: { (args) -> () in
                ...
                self?.delegate?.SingularitySwiftR_ReceiveMessage!(messagemodel)
            ...
            self?.chatHub.on("chatUserStatusChanged", callback: { (args) -> () in
                ...
                self?.delegate?.SingularitySwiftR_ChatUserStatusChanged!(userstatemodel)
            ...
            connection.starting = { [weak self] in
                print("Starting...")
                self!.SingularitySwiftRDaostate = SingularitySwiftRState.Connecting
                print("SingularitySwiftRstate:\(self!.SingularitySwiftRDaostate)")
            ...
            connection.connected = { [weak self] in
                ...
                self!.SingularitySwiftRDaostate = SingularitySwiftRState.Connected
                print("SingularitySwiftRstate:\(self!.SingularitySwiftRDaostate)")
                self!.delegate?.BreakOrConnectTheConnection!(String(self!.hubConnection.state))
            }
            ...
            connection.reconnected = { [weak self] in
                ...
                self!.SingularitySwiftRDaostate = SingularitySwiftRState.Connected
                print("SingularitySwiftRDaostate:\(self!.SingularitySwiftRDaostate)")
                self!.delegate?.BreakOrConnectTheConnection!(String(self!.hubConnection.state))
            }
            
            connection.disconnected = { [weak self] in
                print("Disconnected.")
                self!.SingularitySwiftRDaostate = SingularitySwiftRState.Disconnected
                self!.delegate?.BreakOrConnectTheConnection!(String(self!.hubConnection.state))
            }
    ...
    */
    func BreakOrConnectTheConnection(){
        ...
        let state:String = String(SingularitySwiftRDaostate)
